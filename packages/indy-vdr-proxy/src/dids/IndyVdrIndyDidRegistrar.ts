...
import type { VdrPoolProxy } from '../vdrProxy/VdrPoolProxy'
import type {
  AgentContext,
  Buffer,
  DidRegistrar,
...
import { VdrPoolConfig } from '../vdrProxy/VdrPoolProxy'
...
export class IndyVdrIndyDidRegistrar implements DidRegistrar {
  ...
  public async create(agentContext: AgentContext, options: IndyVdrDidCreateOptions): Promise<DidCreateResult> {
    ...
    try {
      ...
      const pool = agentContext.dependencyManager.resolve(IndyVDRProxyService).getPoolForNamespace(submitterNamespace)
