...
export class IndyVdrIndyDidResolver implements DidResolver {
  ...
  private async buildDidDocument(agentContext: AgentContext, did: string) {
    ...
    const poolService = agentContext.dependencyManager.resolve(IndyVDRProxyService)
  ...
  private async getPublicDid(pool: VdrPoolProxy, unqualifiedDid: string) {
    ...
    if (!didResponse.result.data) {
      throw new IndyVdrNotFoundError(`DID ${unqualifiedDid} not found in indy namespace ${pool.didIndyNamespace}`)
    }
    return JSON.parse(didResponse.result.data) as GetNymResponseData
  }

  private async getEndpointsForDid(agentContext: AgentContext, pool: VdrPoolProxy, unqualifiedDid: string) {
    try {
