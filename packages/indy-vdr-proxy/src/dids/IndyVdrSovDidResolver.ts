...
export class IndyVdrSovDidResolver implements DidResolver {
  ...
  public async resolve(agentContext: AgentContext, did: string, parsed: ParsedDid): Promise<DidResolutionResult> {
    ...
    try {
      const indyVdrPoolService = agentContext.dependencyManager.resolve(IndyVDRProxyService)
  ...
  private async getEndpointsForDid(agentContext: AgentContext, pool: VdrPoolProxy, did: string) {
    try {
